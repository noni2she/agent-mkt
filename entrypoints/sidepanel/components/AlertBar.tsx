...
type Tone = "danger" | "warning" | "info" | "success" | "account-mismatch";
...
const tones: Record<Tone, { bar: string; icon: string; text: string }> = {
  ...
  "account-mismatch": { bar: "border-[var(--danger-bd)] bg-[var(--warning-soft)]", icon: "text-[var(--danger)]", text: "text-[var(--danger-text)]" },
};

export function AlertBar({ children, tone = "warning", title, icon, actual, expected, className = "", ...props }: AlertBarProps) {
  const t = tones[tone];
  const content = tone === "account-mismatch"
    ? `Threads 分頁登入是 @${actual ?? "unknown"}、active 帳號是 @${expected ?? "unknown"}，請在 Threads 切換到正確帳號`
    : children;
  ...
  return (
    <div role="alert" className={`flex items-start gap-[11px] rounded-[var(--radius-lg)] border px-[14px] py-3 ${t.bar} ${className}`} {...props}>
      ...
      <div className="flex min-w-0 flex-col gap-0.5">
        ...
        {content ? <div className={`opacity-92 [font:var(--fs-sm)/1.55_var(--font-sans)] ${t.text}`}>{content}</div> : null}
      </div>
    </div>
  );
}
