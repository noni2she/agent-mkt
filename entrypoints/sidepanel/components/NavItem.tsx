...
export function NavItem({ icon, label, active = false, count, className = "", ...props }: NavItemProps) {
  return (
    <button
      type="button"
      className={`flex min-w-0 flex-1 cursor-pointer flex-row items-center justify-center gap-1.5 rounded-[var(--radius-md)] border px-2 py-[9px] text-center transition-colors duration-[var(--dur-fast)] ease-[var(--ease-out)] disabled:cursor-not-allowed [font:var(--fw-medium)_var(--fs-sm)/1_var(--font-sans)] ${active ? "border-[var(--success-bd)] bg-[var(--success-soft)] text-[var(--success-text)]" : "border-transparent bg-transparent text-[var(--text-body)] hover:bg-[var(--surface-inset)]"} ${className}`}
      {...props}
    >
