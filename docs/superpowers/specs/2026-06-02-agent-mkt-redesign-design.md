...
| D8 通道 | WebSocket 掛在 **service worker** | content script 的 WS 受頁面 CSP（connect-src）管制、Threads 會擋 → 改由 SW（擴充情境、不受頁面 CSP）持 WS，靠 WS 心跳 keep-alive（Chrome 116+ 重置 idle timer）；content script 經 chrome messaging 由 SW 轉發。agent loop 仍在後端，與 SW 生命週期無關。派工 gated 在「分頁開著 + active 連線」|
...
- WebSocket 掛在 **service worker**（擴充情境，不受頁面 CSP `connect-src` 管制；content script 的 WS 會被 Threads CSP 擋）。靠 WS 心跳 keep-alive（Chrome 116+ WS 訊息重置 SW idle timer）+ 斷線重連。
- **agent loop 在後端、不在 SW**——SW 只做「持 WS + 在 SW↔content script 間轉發指令/結果」這種輕量工作，不受 30秒/5分鐘牆影響。
- content script 經 chrome messaging 收 SW 轉來的指令、做 DOM I/O、回傳結果給 SW。
- 後端派工 gated 在「Threads 分頁開著 + active WS」；無連線則跳過/延後。
- 本機 `ws://localhost` ↔ 雲端 `wss://...` + per-tenant token，差別只在設定（從 chrome.storage 讀），非重寫。
...
| 3 | reply 寫稿：orchestrator + LLMClient + core/prompt + kb 讀 → 產 ReviewItem 入 SQLite | LLM 鏈路（PoC 缺）|
